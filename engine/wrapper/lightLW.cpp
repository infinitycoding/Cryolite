...
CLASS
    ...
    FUNCTION(update_lamp,VOID getInstance(Lamp)->refreshPosition(),1,istobjecttype(Lamp))
    FUNCTION(activat_lamp, VOID getInstance(Lamp)->activate(),1,istobjecttype(Lamp))
    FUNCTION(deactivate_lamp, VOID getInstance(Lamp)->deactivate(),1,istobjecttype(Lamp))
    FUNCTION(dir_lamp, VOID getInstance(Lamp)->directed(LBOOL()),2,istobjecttype(Lamp))
    FUNCTION(is_dir,LRET(1); LDBL(getInstance(Lamp)->isDirected()),1,istobjecttype(Lamp))
    ...
    FUNCTION(std_light, VOID getInstance(Lamp)->setStandart(),1,istobjecttype(Lamp))
    FUNCTION(refresh_light, VOID getInstance(Lamp)->refresh(),1,istobjecttype(Lamp))
